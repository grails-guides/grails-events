...
<2> Instead of annotating the method with `@Publisher` which is usually the recommended approach to publish an event, we do it here manually with `notify` method.
